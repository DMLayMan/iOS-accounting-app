import {TODAY,FIRST,pad,iso,current,periodRange,step,validStep,recordsIn,totals,comparisonRange} from './model.mjs';
...
function apply(value){selection=value;panel=false;visibleCount=10;render();$('#workspace').scrollTop=0;$('#chooseDate').focus({preventScroll:true});announce(`已切换到${selectedTitle()}，统计与流水共用此时间。`);}
function renderTime(){
  const canStep=['month','year'].includes(selection.kind);
  const unit=selection.kind==='year'?'年':'月';
  $('#timebar').innerHTML=`${canStep?`<button class="step" data-step="-1" aria-label="上一个${unit}" ${validStep(selection,-1)?'':'disabled'}>${icon('left')}</button>`:''}<button class="date-label" id="chooseDate" aria-expanded="${panel}" aria-controls="periodPanel">${selectedTitle()}${icon('down')}</button>${canStep?`<button class="step" data-step="1" aria-label="下一个${unit}" ${validStep(selection,1)?'':'disabled'}>${icon('right')}</button>`:''}`;
...
document.addEventListener('click',event=>{
  ...
  if(button.dataset.step){apply(step(selection,Number(button.dataset.step)));return;}
  ...
  switch(button.id){
    ...
    case 'compact':case 'dark':case 'large':$('.device').classList.toggle(button.id);button.setAttribute('aria-pressed',$('.device').classList.contains(button.id));break;
    case 'retry':load();break;
  }
});
document.addEventListener('keydown',event=>{if(event.key==='Escape'&&panel){panel=false;renderTime();$('#chooseDate').focus();}});
async function load(){
